...
class StiSupabaseUser extends BaseAuthUser {
  StiSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> stiSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StiSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
